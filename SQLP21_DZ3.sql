...
update history_position hp 
set person_position_id = (select person_position_id 
							from person_position pp 
							left join person pe using(person_id) 
							left join position po using(position_id) 
							where pe.person_name = 'Mary Roberts' and pp.salary = 23500.)
where hp.person_id = 1 and old_salary=17000.;
update history_position hp 
set person_position_id = (select person_position_id 
							from person_position pp 
							join person pe using(person_id) 
							join position po using(position_id) 
							where pe.person_name = 'Mary Roberts' and pp.salary = 23500.)
where hp.person_id = 1 and old_salary=18500.;
update history_position hp 
set person_position_id = (select person_position_id 
							from person_position pp 
							join person pe using(person_id) 
							join position po using(position_id) 
							where pe.person_name = 'Everett Garcia' and pp.salary = 27990.)
where hp.person_id = 3 and old_salary=12000.;
commit;

-----------создание материализованного представления для просмотра всей информации по сотрудникам, включая архивную---------

create materialized view IF NOT exists all_person (
...
select 
	...
	d.department_address  as "Адрес департамента (город, улица, номер дома)", 
	s.supervisor_name as "Руководитель", 
	CONCAT(pp.effective_from,' - ', 'now')  as "Период работы",
	pp.salary as "Зарплата1"
from person p
...
ps.position_name as "Должность", 
d.department_name  as "Департамент", 
coalesce(hda.old_department_address, (select department_address from department d2 where d2.department_id = d.department_id))  as "Адрес департамента",
s.supervisor_name  as "Руководитель", 
CONCAT(hp.date_begin, ' - ', hp.date_end) as "Период работы",
hp.old_salary as "Зарплата1"
from person p
...
-- check_position_id() - наличие в таблице position,
-- check_supervisor_id() - наличие в таблице supervisor,
-- check_person_id() - наличие в таблице person,
-- check_department_id() - наличие в таблице депарамент
-- check_person_id_in_history_person_address() - наличие в таблице  history_person_address
-- check_department_id_in_history_department_address() - наличие в таблице  history_department_address
 ...
 * view текущей записи по сотруднику с заданным person_id
 */
create or replace function current_person (pers int) RETURNS table(
person_name text, 
date_birth date,
address text, 
email text,
...
join department d using(department_id)
left join supervisor s using(supervisor_id)
where p.person_id = pers;

$$LANGUAGE sql;
--test
select * from current_person(1);
--archive_person-------------------------------------------------------------------------------------------------------------------------------
/* Вспомогательная функция, возвращает
 * view архивных записей по сотруднику с заданным person_id
 */
create or replace function archive_person (pers int) RETURNS table(
person_name text, 
date_birth date,
address text, 
email text,
...
select 
	p.person_name as "Фамилия и имя сотрудника", 
	p.date_birth as "Дата рождения", 
	coalesce(hpa.old_person_address , (select address from person where person_id = pers) ) as "Адрес проживания", 
	p.email as "электронная почта",
	ps.position_name as "Должность", 
	d.department_name  as "Департамент", 
	coalesce(hda.old_department_address, get_department_address(pers))  as "Адрес департамента",
	s.supervisor_name  as "Руководитель", 
	CONCAT(hp.date_begin, ' - ', hp.date_end) as "Период работы",
...
left join history_department_address hpd using(department_id)
left join supervisor s on hp.old_supervisor_id = s.supervisor_id
where s.supervisor_name is not null and p.person_id = pers
order by 1;

$$LANGUAGE sql;
--test
select * from archive_person(1);
...
/*
 * Триггерная функция срабатывает на обновление текущего адреса сотрудника. Добавление запрещено, т.к у сотрудника может быть только один текущий адрес
 */
declare last_updated timestamp;
old_person_address_ text;
begin
	last_updated = current_timestamp;
	old_person_address_ = (select address from person where person_id =new.person_id);
...
select * from history_person_address where person_id = 1;
...
begin
 	...
 	last_updated = current_timestamp;
 	old_department_address_ = (select department_address from department where department_id =new.department_id);
...
select * from history_department_address where department_id = 1;
...
--тесты-------------------------------------------------------------------------------------------------------
--1. Прежняя должность, новая зарплата, новый руководитель. Записи в истории нет
begin;
savepoint mysavepoint1;
select * from current_person(4) ;
select *  from archive_person(4);
...
--3. Новая должность, новая зарплата, новый руководитель. Записи в истории нет
begin;
savepoint mysavepoint1;
select * from current_person(4) ;
select *  from archive_person(4);
...
rollback to mysavepoint1;
commit;
--4. новая должность, новая зарплата,новый руководитель Записи в истории есть
begin;
savepoint mysavepoint1;
select * from current_person(1) ;
select *  from archive_person(1);
